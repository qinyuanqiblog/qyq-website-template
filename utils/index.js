 ...
 * @Last Modified time: 2020-11-09 14:56:13
 *-------------------------------------------------------------
 */
...
// const modulesContext = require.context('./', true, /Route\.js$/)
//  动态引入文件，以Array格式返回
export function requireArrayModules(modulesContext) {
  ...
  return function () {
    return moduleNames.reduce(function (routes, moduleName) {
...
//  动态引入文件，以json格式返回
export function requireJsonModules (modulesContext) {
  let chunks = modulesContext.keys().reduce(function (modules, key) {
    modules[key.replace(/(^\.\/)|(\.js$)/g, '')] = modulesContext(key).default
    return modules
  }, {})
...
export function createModules(modulesContext) {
  var chunks = modulesContext.keys().reduce(function (modules, key) {
    modules[key.replace(/(^\.\/)|(\.js$)/g, '')] = modulesContext(key).default
    return modules
  }, {})
  var moduleNames = Object.keys(chunks)
  return function (handleRequest, API) {
    return moduleNames.reduce(function (modules, moduleName) {
